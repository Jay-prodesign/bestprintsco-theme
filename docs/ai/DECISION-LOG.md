...
| PI-DEC-003 | 2026-08-14 | Use Proposed DescriptionHtml as the authorized Capri interim runtime baseline before classifying description drift. | PCC Change Log row 1363 proves the earlier interim write was preserved with 72/72 post-read/protected equality; fresh live values match Proposed exactly 68/68. | active |
| PI-DEC-004 | 2026-08-14 | Retract the prior 68-row SOURCE_CONFLICT conclusion as a baseline-selection false positive; do not run the final mutation in this reconciliation. | A=68, B=0, C=0 under exact and normalized Proposed-first comparison; Shopify mutation count remains zero. | active |
